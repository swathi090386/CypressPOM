class ProviderDoctorsCliniciansPage{
    ...
    radiusEle = (':nth-child(1) > .mat-form-field-should-float > .mat-form-field-wrapper > .mat-form-field-flex > .mat-form-field-infix');   
    searchproviderEle = (".display-flex > .mat-focus-indicator > .mat-button-wrapper");
    categoryName = ('.mat-input-element'); 
    specialityName = ('#mat-input-26');
    radiusinEle = ('#mat-input-4');
    distanceEle = '[name="distance"]';
    ...
    specialityNameEle = ('#mat-option-0 > .mat-option-text');
   ...
   zipcodeEle1 = ("//input[@id='mat-input-2']");      
...
enterBusinessName(businessname){
    cy.get(this.businessnameEle).type(businessname);
}
...
enterZipCode1(zipcode1){
  cy.xpath(this.zipcodeEle1).clear().wait(1000);
  cy.xpath(this.zipcodeEle1).type(zipcode1).click({ multiple: true }).click({ force: true });
}
enterZipCode2(zipcode2){
  cy.xpath(this.zipcodeEle2).clear().wait(1000);
  cy.xpath(this.zipcodeEle2).type(zipcode2);
}
enterZipCode3(zipcode3){
  cy.xpath(this.zipcodeEle2).clear().wait(1000);
  cy.xpath(this.zipcodeEle2).type(zipcode3);
}
enterZipCode4(zipcode4){
  cy.xpath(this.zipcodeEle2).clear().wait(1000);
  cy.xpath(this.zipcodeEle2).type(zipcode4);
}
enterZipCode5(zipcode5){
  cy.xpath(this.zipcodeEle2).clear().wait(1000);
  cy.xpath(this.zipcodeEle2).type(zipcode5);
}
enterZipCode6(zipcode6){
  cy.xpath(this.zipcodeEle2).clear().wait(1000);
  cy.xpath(this.zipcodeEle2).type(zipcode6);
}
...
clickZipSearch(){
    cy.get(this.zipsearchEle, { timeout : 3000 }).as('btn').click({ force: true });
}
clickCountyState(){
...
selectCategorydropdown(){
    cy.get(this.categoryName).should('be.visible').click({ multiple: true });
}
...
clickHospicecare(){
    cy.get('.mat-option-text').contains('Hospice care').click({ force: true });
}
selectSpecialtydropdown(){
    cy.get(this.specialitydropdownEle).should('be.visible').click();
}
selectCategory(categoryName){
   cy.get(this.categoryName).click({ multiple: true }).click({ force: true });
   }
selectDefaultCategory(categoryName){
   cy.get('.mat-input-element').should('be.visible');
    ...
    clickProviderFilter(){
     cy.get('mat-expansion-panel-header').contains('Provider filters').should('be.visible').click({ force: true });
     cy.wait(2000)
  }
   enterDistance(distance){
        cy.get(this.distanceEle).click({force:true}).type(distance);
    }
    clickGender(){
